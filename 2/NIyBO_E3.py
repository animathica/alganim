...
class E3(MovingCameraScene):
    def construct(self):
        ...
        texto0 = Tex("\\mbox{*", "Ver el ", "Ejercicio 1.2 ", "del video ", "\\emph{Ortogonalización y ortonormalización (Teorema de Gram-Schmidt) }} \\hspace{20cm}", "de Animathica.").scale(0.55).to_edge(DOWN)
        texto0[0].set_color(AMARILLO)
        texto0[2].set_color(AZUL)
        texto0[4].set_color(AMARILLO)
        texto0[5].shift(3.75*LEFT)
        ...
        #Animaciones
        self.next_section("...que distinguen al vector nulo", skip_animations=SKIP_DEFAULT)
        self.play(Write(texto0), run_time=1.5)
        self.wait(1.5)
        ...
        self.play(Write(texto3), run_time=2.5)
        self.wait()

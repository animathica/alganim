...
class SE1(Scene):
    def construct(self):
        ...
        group_1 = VGroup(base_beta, base_propiedades,
                         combination, coeficientes_c).scale(0.9)
...
5.5*LEFT)
        ...
        def gen1(Vec1, Vec2, Lab1, Lab2):
            ...
            Vec1c = DashedArrow(Vec2.get_end(), Vec2.get_end(
            )+A1*RIGHT+A2*UP, buff=0, color=Vec1.get_color()).set_fill(opacity=0.5)
            Vec2c = DashedArrow(Vec1.get_end(), Vec1.get_end(
            )+B1*RIGHT+B2*UP, buff=0, color=Vec2.get_color()).set_fill(opacity=0.5)
            self.play(Create(VGroup(Vec1c, Vec2c)))
            # Vector resultante de la combinación lineal.
            VecRCL = DashedArrow((0, 0, 0), Vec2c.get_end(), buff=0,
                                 color=MAGENTA).set_fill(opacity=1)
            ...
            def upd_for_vec2_1(obj):
                t = vt1.get_value()
                NewVec2 = Arrow((0, 0, 0), (t*B1, t*B2, 0),
                                buff=0, color=Vec2.get_color())
                obj.become(NewVec2)
            # Primera función para cambios de Vec2c.

            def upd_for_vec2c_1(obj):
                t = vt1.get_value()
                NewVec2c = DashedArrow(Vec1.get_end(), Vec1.get_end(
                )+(t*B1, t*B2, 0), buff=0, color=Vec2.get_color()).set_fill(opacity=0.5)
                obj.become(NewVec2c)
            # Primera función para cambios de VecRCL.

            def upd_for_vecrcl_1(obj):
                t = vt1.get_value()
                NewVecRCL = DashedArrow((0, 0, 0), Vec2c.get_end(
                ), buff=0, color=MAGENTA).set_fill(opacity=1)
                obj.become(NewVecRCL)
            # Primera línea usada.
            Linea1 = Line(Copia1.get_end()+(0.001, 0.001, 0)+B1*RIGHT+B2*UP,
                          Vec2c.get_end(), color=MAGENTA).set_fill(opacity=0.5)
            # Segunda línea usada.
            Linea2 = Line(Vec1.get_end(), Vec1.get_end()-(0.01*B1, 0.01 *
                                                          B2, 0), color=MAGENTA).set_fill(opacity=0.5)
            # Función para cambiar tamaño de las líneas.

            def upd_for_linea(obj):
                t = vt1.get_value()
                new_linea = Line(Copia1.get_end()+(0.001, 0.001, 0)+B1*RIGHT+B2*UP,
                                 Vec1.get_end()+(t*B1, t*B2, 0), color=MAGENTA).set_fill(opacity=0.5)
            ...
            ParInf1 = Tex(")").move_to(Num_Lin_1.get_start() +
                                       0.04*UP).scale(1).rotate(-np.pi/2)
            ParSup2 = Tex(")").move_to(Num_Lin_2.get_end() +
                                       0.04*DOWN).scale(1).rotate(np.pi/2)
            ParInf2 = Tex(")").move_to(Num_Lin_2.get_start() +
                                       0.04*UP).scale(1).rotate(-np.pi/2)
            # Símbolos de infinito
            Inf_1 = Tex("$ \\infty $").move_to(Num_Lin_1.get_end()+0.25*UP).scale(0.6)
            MInf_1 = Tex("$ - \\infty $").move_to(Num_Lin_1.get_start() +
                                                  0.25*DOWN).scale(0.6)
            Inf_2 = Tex("$ \\infty $").move_to(Num_Lin_2.get_end()+0.25*UP).scale(0.6)
            MInf_2 = Tex("$ - \\infty $").move_to(Num_Lin_2.get_start() +
                                                  0.25*DOWN).scale(0.6)
            # Linea con que se colorea primera recta numérica.
            LINEA_1 = Line(Num_Lin_1.number_to_point(
            ...
            def upd_for_LINEA_1(obj):
                ...
                if t > 0:
                    if Num_Lin_1.number_to_point(t)[1] > obj.get_end()[1]:
                        new_LINEA = Line(obj.get_start(), Num_Lin_1.number_to_point(
                            t), color=ROJO, stroke_width=15)
                    else:
                        return None
                elif t < 0:
                    if Num_Lin_1.number_to_point(t)[1] < obj.get_start()[1]:
                        new_LINEA = Line(Num_Lin_1.number_to_point(
                            t), obj.get_end(), color=ROJO, stroke_width=15)
                    else:
                        return None
                elif t == 0:
                    return None
            ...
            def upd_for_FLECHA_1(obj):
                t = vt1.get_value()
                new_FLECHA = Arrow(0.25*RIGHT, 0.25*LEFT, buff=0).set_height(
                    0.25).next_to(Num_Lin_1.number_to_point(t), RIGHT, buff=0.1)
            ...
            def upd_for_LINEA_2(obj):
                ...
                if t > 0:
                    if Num_Lin_2.number_to_point(t)[1] > obj.get_end()[1]:
                        new_LINEA = Line(obj.get_start(), Num_Lin_2.number_to_point(
                            t), color=AZUL, stroke_width=15)
                    else:
                        return None
                elif t < 0:
                    if Num_Lin_2.number_to_point(t)[1] < obj.get_start()[1]:
                        new_LINEA = Line(Num_Lin_2.number_to_point(
                            t), obj.get_end(), color=AZUL, stroke_width=15)
                    else:
                        return None
                elif t == 0:
                    return None
            ...
            def upd_for_FLECHA_2(obj):
                t = vt1.get_value()
                new_FLECHA = Arrow(0.25*RIGHT, 0.25*LEFT, buff=0).set_height(
                    0.25).next_to(Num_Lin_2.number_to_point(t), RIGHT, buff=0.1)
                obj.become(new_FLECHA)
            ...
            def upd_for_vec1_1(obj):
                t = vt1.get_value()
                NewVec1 = Arrow((0, 0, 0), (t*A1, t*A2, 0),
                                buff=0, color=Vec1.get_color())
                obj.become(NewVec1)
            # Primera función para cambios de Vec1c.

            def upd_for_vec1c_1(obj):
                t = vt1.get_value()
                NewVec1c = DashedArrow(Vec2.get_end(), Vec2.get_end(
                )+(t*A1, t*A2, 0), buff=0, color=Vec1.get_color()).set_fill(opacity=0.5)
                obj.become(NewVec1c)
            # Segunda función para cambios de VecRCL.

            def upd_for_vecrcl_2(obj):
                t = vt1.get_value()
                NewVecRCL = DashedArrow((0, 0, 0), Vec2.get_end(
                )+(t*A1, t*A2, 0), buff=0, color=MAGENTA).set_fill(opacity=1)
                obj.become(NewVecRCL)
            ...
            def upd_for_plano(obj):
                ...
                vert3 = (t*A1, t*A2, 0)
                vert4 = (t*A1, t*A2, 0)
                New_plano = Polygon(vert1, vert2, vert3, vert4,
                                    stroke_width=0).set_fill(MAGENTA_CLARO, opacity=1)
            ...
            self.remove_foreground_mobjects(
                LINEA_1, LINEA_2, Num_Lin_1, Num_Lin_2, Inf_1, MInf_1, Inf_2, MInf_2, ParInf1, ParInf2, ParSup1, ParSup2)
            self.play(FadeOut(VGroup(Plano1, Plano2)), FadeOut(Text14_3.group), FadeOut(VGroup(
                VecRCL, Punto, LINEA_1, LINEA_2, Num_Lin_1, Num_Lin_2, Inf_1, MInf_1, Inf_2, MInf_2, ParInf1, ParInf2, ParSup1, ParSup2)))
        ...
        def encontrar_escalares(vec1, vec_obj1, text_A, vec2, vec_obj2, text_B, vec_objetivo, text_vec_objetivo):
            ...
            par_inf = Tex(")").move_to(number_line.get_start()+0.04 *
                                       UP).scale(1).rotate(-np.pi/2).scale(0.8)
            # Símbolos de infinito para la recta numérica
            inf = Tex("$ \\infty $").move_to(number_line.get_end()+0.25*UP).scale(0.6)
            minf = Tex("$ - \\infty $").move_to(number_line.get_start() +
                                                0.25*DOWN).scale(0.6)
            # Flechas que apuntan al valor de los escalares
            pointer_vec_1 = Arrow(start=ORIGIN, end=RIGHT,
                                  max_tip_length_to_length_ratio=0.4)
            label_1 = MathTex("c_1").add_updater(
                lambda m: m.next_to(pointer_vec_1, 0.3*LEFT))  # MODIF
            pointer_vec_2 = Arrow(start=ORIGIN, end=LEFT,
                                  max_tip_length_to_length_ratio=0.4)
            label_2 = MathTex("c_2").add_updater(
                lambda m: m.next_to(pointer_vec_2, 0.3*RIGHT))  # MODIF
            ...
            label3 = VGroup(plus_sign, label_sum1, label_sum2, label_sum3,
                            label_sum4).move_to(vec_AplusB.get_end()+dist_3)
            # Otros símbolos utilizados
            equal_sign = MathTex("=")
            # Vectores fantasma para la suma
            fantasma_flecha1 = DashedArrow(
                vec2, vec1_plus_vec2, buff=0, stroke_opacity=0.5, color=col1)
            fantasma_flecha2 = DashedArrow(
                vec1, vec1_plus_vec2, buff=0, stroke_opacity=0.5, color=col2)
            ...
            def upd_for_vec3(obj):
                new_vec = Arrow((0, 0, 0), t1.get_value()*vec1 +
                                t2.get_value()*vec2, buff=0, color=col3)
                obj.become(new_vec)
            ...
            def upd_for_fantasma_flecha1(obj):
                new_vec = DashedArrow(vec_obj2.get_end(), vec_AplusB.get_end(
                ), buff=0, color=col1, stroke_opacity=0.5)
                obj.become(new_vec)

            def upd_for_fantasma_flecha2(obj):
                new_vec = DashedArrow(vec_obj1.get_end(), vec_AplusB.get_end(
                ), buff=0, color=col2, stroke_opacity=0.5)
                obj.become(new_vec)
        ...
        text_vec = MathTex("\\vec{v}", color=NARANJA).move_to(
            vec_.get_end()+0.4*vec/(np.linalg.norm(vec)))
        text_A = MathTex("\\vec{b}_1", color=AZUL).move_to(
            vecA.get_end()+0.4*vec_A/(np.linalg.norm(vec_A)))
        text_B = MathTex("\\vec{b}_2", color=ROJO).move_to(
...
class SE3(Scene):
    def construct(self):
        ...
        # Ayuda para las transformaciones
        base_beta_dimK_4 = Tex("$,\\dots,$", "texto auxiliar")
        base_beta_dimK_5 = vk_k_ntup.copy()
        base_beta_dimK_6 = Brace(base_beta_dimK_5, RIGHT)
        # Ayuda para las transformaciones
        base_beta_dimK_7 = Tex(" base de ", "texto auxiliar")
        ...
        entries_vk1_1_ntup = [["v_{1}^1"], ["v_{2}^1"],
                              ["\\vdots"], ["v_{k}^1"], ["v_{k+1}^1"]]
        vk1_1_ntup = Matrix(entries_vk1_1_ntup,
                            left_bracket="\\big(",
                            right_bracket="\\big)")

        entries_vk1_k_ntup = [["v_{1}^k"], ["v_{2}^k"],
                              ["\\vdots"], ["v_{k}^k"], ["v_{k+1}^{k}"]]
        vk1_k_ntup = Matrix(entries_vk1_k_ntup,
                            left_bracket="\\big(",
                            right_bracket="\\big)")

        entries_vk1_k1_ntup = [["v_{1}^{k+1}"], ["v_{2}^{k+1}"],
                               ["\\vdots"], ["v_{k}^{k+1}"], ["v_{k+1}^{k+1}"]]
        ...
        # Ayuda para las transformaciones
        base_beta_dimK1_4 = Tex("$,\\dots,$", "texto auxiliar")
        ...
        # Ayuda para las transformaciones
        base_beta_dimK1_9 = Tex(" base de ", "texto auxiliar")
        ...
        #Alineacion de igualdades
        group_dimK1_ntuplas[0][:].align_to(
            group_dimK_ntuplas[0][:], UP).align_to(group[0][:], LEFT)
        group_dimK1_ntuplas[2][:].align_to(
            group_dimK_ntuplas[2][:], UP).align_to(group[2][:], LEFT)

        #______________________ dim(V) = k + 2

        entries_vk2_ntup = [["v_{1}"], ["v_{2}"], [
            "\\vdots"], ["v_{k}"], ["v_{k+1}"], ["v_{k+2}"]]
        vk2_ntup = Matrix(entries_vk2_ntup,
                          left_bracket="\\big(",
                          right_bracket="\\big)").set_column_colors(AZUL)

        entries_vk2_1_ntup = [["v_{1}^1"], ["v_{2}^1"], [
            "\\vdots"], ["v_{k}^1"], ["v_{k+1}^1"], ["v_{k+2}^1"]]
        vk2_1_ntup = Matrix(entries_vk2_1_ntup,
                            left_bracket="\\big(",
                            right_bracket="\\big)")

        entries_vk2_k_ntup = [["v_{1}^k"], ["v_{2}^k"], [
            "\\vdots"], ["v_{k}^k"], ["v_{k+1}^{k}"], ["v_{k+2}^{k}"]]
        vk2_k_ntup = Matrix(entries_vk2_k_ntup,
                            left_bracket="\\big(",
                            right_bracket="\\big)")

        entries_vk2_k1_ntup = [["v_{1}^{k+1}"], ["v_{2}^{k+1}"],
                               ["\\vdots"], ["v_{k}^{k+1}"], ["v_{k+1}^{k+1}"], ["v_{k+2}^{k+1}"]]
        vk2_k1_ntup = Matrix(entries_vk2_k1_ntup,
                             left_bracket="\\big(",
                             right_bracket="\\big)")

        entries_vk2_k2_ntup = [["v_{1}^{k+2}"], ["v_{2}^{k+2}"],
                               ["\\vdots"], ["v_{k}^{k+2}"], ["v_{k+1}^{k+2}"], ["v_{k+2}^{k+2}"]]
        ...
        # Ayuda para las transformaciones
        base_beta_dimK2_4 = Tex("$,\\dots,$", "texto auxiliar")
        ...
        # Ayuda para las transformaciones
        base_beta_dimK2_11 = Tex(" base de ", "texto auxiliar")
        ...
        #Alineacion de igualdades
        group_dimK2_ntuplas[0][:].align_to(
            group_dimK1_ntuplas[0][:], UP).align_to(group[0][:], LEFT)
        group_dimK2_ntuplas[2][:].align_to(
            group_dimK1_ntuplas[2][:], UP).align_to(group[2][:], LEFT)
        ...
        sis_row6 = MathTex(r"v_{k+2}", "=", "c_1", "v_{k+2}^{1}", "+", r"\cdots", "+", "c_{k}", "v_{k+2}^{k}", "+", "c_{k+1}", "v_{k+2}^{k+1}",
                           "+", "c_{k+2}", "v_{k+2}^{k+2}")
        sist_group = VGroup(sis_row1, sis_row2, sis_row3, sis_row4,
                            sis_row5, sis_row6).scale(0.5).move_to(3.5*RIGHT+3*UP)

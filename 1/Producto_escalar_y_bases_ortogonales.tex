...
\noindent Por último, si una base cumple con ser un conjunto ortogonal, diremos que es una \emph{base ortogonal}. Dado que se puede demostrar que un conjunto ortogonal es linealmente independiente si, y sólo si, no contiene al vector nulo\footnote{Escribir ``Ver Ejercicio 1.5.'' como pie de página.} entonces, por el Ejercicio 1.2 mencionado anteriormente, tenemos que un conjunto $\Gamma$ es una base ortogonal de $V$ si se cumplen las condiciones siguientes\footnote{Tal vez quede mejor cambiando de lugar los dos casos de la expresión $\langle \vec{g}_j , \vec{g}_i \rangle$ aquí y en lo que sigue, pero no estoy seguro aún.}

\begin{align*}
    \Gamma=\{\vec{g}_1,...,\vec{g}_k\} \text{ es base \emph{ortogonal} de } V \text{ si}& & &\\
    \\
    \langle \vec{g}_j , \vec{g}_i \rangle = \begin{cases} \langle\vec{g_i}, \vec{g}_i\rangle\neq0 \ \ \text{si} \ \ j = i \\ \quad 0 \quad \ \ \ \ \ \ \ \ \ \text{si} \ \ j\neq i, \end{cases}&\langle\Gamma\rangle = V. & &\\
\end{align*}

Para poder interpretar la ortogonalidad geométricamente, es de gran utilidad ver un ejemplo pero, para eso, necesitamos ver antes un ejemplo de producto escalar. A continuación, veremos un ejemplo en el espacio vectorial real $\mathbb{R}^2$ conocido comunmente como \emph{producto punto}, cuya regla de correspondencia es la siguiente. Veamos que esta operación efectivamente cumple las propiedades de producto escalar, demostrándolas una por una.
\begin{align*}
...
Cabe resaltar que este producto punto, que acabamos de demostrar que es un producto escalar en $\mathbb{R}^2$, puede generalizarse fácilmente al espacio $\mathbb{R}^n$, e inclusive al espacio vectorial complejo $\mathbb{C}^n$. La demostración de que la operación en pantalla es un producto escalar queda como ejercicio para quien quiera hacerlo.
...
Pregunta 1.8 En un espacio vectorial \textbf{complejo} con producto escalar, ¿cómo puedes interpretar la operación de simetría conjugada \emph{geométricamente}? (Sugerencia: Considera el espacio vectorial complejo $\mathbb{C}$.)
